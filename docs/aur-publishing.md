...
cd ~/aur/roon-proton
cp ~/roon-wine/{PKGBUILD,.SRCINFO,roon-proton.install,roon-wine,roon-wine.desktop,roon-umu-run,roon-proton.modules-load.conf,roon-proton.modprobe.conf,LICENSE} .
cp ~/roon-wine/roon-proton.{niri.kdl,hyprland.conf,hyprland.lua} .
git add PKGBUILD .SRCINFO roon-proton.install roon-wine roon-wine.desktop roon-umu-run \
  roon-proton.modules-load.conf roon-proton.modprobe.conf \
  roon-proton.* LICENSE
git commit -m 'Initial import: roon-proton 0.2.0-5'
